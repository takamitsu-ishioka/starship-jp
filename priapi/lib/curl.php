...
class Curl {
    ...
    private function SendRequest($proxy, $proxy_port, $url, $auth, $body, $headers, $post_flag) {
        try {
            $this->status_code = 0;
    ...
    public function Post($proxy, $proxy_port, $url, $auth, $body, $headers = []) {
        return $this->SendRequest($proxy, $proxy_port, $url, $auth, $body, $headers, TRUE);
    }

    public function Get($proxy, $proxy_port, $url, $auth, $body, $headers = []) {
        return $this->SendRequest($proxy, $proxy_port, $url, $auth, $body, $headers, FALSE);
    }

    public function SendJSON($method, $url, $headers, $data) {
        try {
            ...
            if (!empty($data)) {
                curl_setopt($this->handle, CURLOPT_POSTFIELDS, $data_json);
            }
            $response = curl_exec($this->handle);
            $this->status_code = curl_getinfo($this->handle, CURLINFO_HTTP_CODE);
            if ($this->status_code != 204 && !$response) {
                $this->Error();
            }
            curl_close($this->handle);
            return $response;
