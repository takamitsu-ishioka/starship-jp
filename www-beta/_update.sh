...
function difference() {
    ...
    mkdir "$tmp_dir"
    if [ $? -ne 0 ]; then
        echo2 "$_script: Can't create $tmp_dir"
        return 1
    fi
...
function main() {
    ...
    local exclude=(
        ...
        --exclude='audio/'
        --exclude='backup*/'
        --exclude='img/*/'              # img のサブディレクトリはすべて除外。普通のファイルは更新。 */
        --include='css/common.css'
        --exclude='css/*'               # css の下は common.css 以外を除外。 */
        --exclude='joli/'
        --include='js/actions.js'
        --include='js/common.js'
        --include='js/paypal.js'
        --include='js/website.js'
        --exclude='js/*'                # js の下は actions.js, common.js, paypal.js, website.js 以外を除外 */
        --exclude=',*'
        --exclude='var_dump*'
        --exclude='*.log'
        --exclude='*.logs'
