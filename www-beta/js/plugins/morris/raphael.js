...
/*
問題:
Error: <rect> attribute width: A negative value is not valid. ("-1.4464285714285714")

所見:
・これ↓を表示するとエラーが出なくなる。表示をやめると再現する。
console.log('>>>>>>>key', key);
console.log('>>>>>>>attr[key]', attr[key]);
console.log('>>>>>>>Str(attr[key])', Str(attr[key]));
・デバッグ中に発見したが、この場所は setTimeout() の中から呼ばれている。
・このバグを発見したきっかけはファイルのダウンロードだったので、てっきりそれに関係があると思ったが、実はウィンドウをリサイズするだけで確実にエラーが発生する。

結論：
タイミング依存臭い。
プンプン臭う。
画面各部のレンダリングが終わる前、つまりサイズの再計算が終わる前におかしなデータを見てグラフの各部のサイズを決めようとしているんだろう。
だめじゃん raphael 君。

対策：無い。エラーが出ても動くので放置する。
*/
